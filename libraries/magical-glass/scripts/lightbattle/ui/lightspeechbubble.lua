local LightSpeechBubble, super = Class(Object)
...
function LightSpeechBubble:onRemoveFromStage(stage)
...
function LightSpeechBubble:advance()
    self.text:advance()
end

function LightSpeechBubble:setText(text, callback, line_callback)
...
function LightSpeechBubble:setAuto(auto)
    self.text.auto_advance = auto or false
end

function LightSpeechBubble:setAdvance(advance)
    self.text.can_advance = advance or false
end

function LightSpeechBubble:setSkippable(skippable)
    self.text.skippable = skippable or false
end

function LightSpeechBubble:setCallback(callback)
    self.advance_callback = callback
    self.text.advance_callback = callback
end

function LightSpeechBubble:setLineCallback(callback)
    self.line_callback = callback
    self.text.line_callback = callback
end

function LightSpeechBubble:setRight(right)
    self.right = right
...
function LightSpeechBubble:isTyping()
    return self.text:isTyping()
end

function LightSpeechBubble:isDone()
    return self.text:isDone()
end

function LightSpeechBubble:update()
    super.update(self)
...
function LightSpeechBubble:getBorder()
...
function LightSpeechBubble:getDebugRectangle()
...
function LightSpeechBubble:getSprite(name)
...
function LightSpeechBubble:getSpriteSize(name)
...
function LightSpeechBubble:getTailWidth()
    local tail_width, _ = self:getSpriteSize("tail")
    return tail_width
end

function LightSpeechBubble:updateSize()
    if self.auto then
