    ...
    for i,battler in ipairs(self.attackers) do
        ...
        for i = 1, lane.weapon and lane.weapon.getLightBoltCount and lane.weapon:getLightBoltCount() or 1 do
            ...
            if i == 1 then
                ...
                if lane.direction == "left" then
                    bolt = LightAttackBar(start_x + (lane.weapon and lane.weapon.getLightMultiboltVariance and lane.weapon:getLightMultiboltVariance(i - 1) or 24 + 40 * (i - 1)), 319, battler, scale_y)
                else
                    bolt = LightAttackBar(start_x - (lane.weapon and lane.weapon.getLightMultiboltVariance and lane.weapon:getLightMultiboltVariance(i - 1) or 24 + 40 * (i - 1)), 319, battler, scale_y)
                end
